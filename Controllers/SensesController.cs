...
namespace Behavior.Controllers
{
    public class SensesController : Controller
    {
        ...
        {
            return Ok(await _context.Senses.ToListAsync());
        }
        ...
        public async Task<IActionResult> Create([FromBody] Sense sense)
        {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(int id, [FromBody] Sense sense)
        {
            if (id != sense.Id)
            {
                return NotFound();
